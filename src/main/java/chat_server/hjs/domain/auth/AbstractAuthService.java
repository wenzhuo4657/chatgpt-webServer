...
public abstract class AbstractAuthService implements IAuthService {
    ...
    public AuthStateEntity doLogin(String code) {
        ...
        AuthStateEntity res =this.checkCode(code);
        if (!res.getCode().equals(AuthTypeVO.A0000.getCode())){
            return  res;
        }

        Map<String,Object> chaim =new HashMap<>();
        chaim.put(AuthEnum.openId_KEY,res.getOpenId());
        String token=encode(res.getOpenId(), 7 * 24 * 60 * 60 * 1000,chaim);
        res.setToken(token);
